...
const AllCategory = () => {
    ...
    const [filterData, setFilterData] = useState<productDataType[]>()
    ...
    const filterProductCategory = useCallback((category: string) => {

        async function fetchDataByCategory() {
            const response = await fetch(`https://dummyjson.com/products/category/${category}`)
            const value = await response.json()
            setFilterData(value.products)
